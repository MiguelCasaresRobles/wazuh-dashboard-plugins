...
async function appendWindowRecountHint(
  ...
  try {
    ...
    // applySafetyValves forces track_total_hits to `true` (an EXACT count, see its own comment in
    // guardrails.ts -- the previous NUMERIC 10000 cap was removed precisely because it made
    // counts under-report), so this recount normally comes back with relation:'eq'. The 'gte'
    // branch is kept as a defensive fallback: any response that does report a lower bound must be
    // worded "at least N", never as an exact figure -- a fabricated count in the one feature that
    // exists to stop counts being misstated.
...
function appendSubTechniqueSplitHint(digest: Digest): void {
  ...
  // `key` is `unknown` since #8909 widened it for multi_terms (array) / composite (object) bucket
  // keys -- a technique id only ever arrives as the scalar STRING case, so narrow rather than
  // coerce: `String()`-ing an array/object key here would let "[object Object]" reach the regex.
  if (
    !keys.some(key => typeof key === 'string' && SUB_TECHNIQUE_KEY_RE.test(key))
  ) {
    return;
  }
  const hint =
...
async function executeIndexerRequest(
  ...
  try {
    ...
    // Carries BOTH `body` (#8920, for deriveColumns/`extractAggFields`) and `assumptionNote`
    // (#8913, from the `resolveParams` hook) -- see buildDigest's own signature doc.
