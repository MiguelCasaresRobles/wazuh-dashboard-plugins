...
export const FIELD_POLICY_DEFAULTS: FieldPolicyEntry[] = [
  ...
  // actually needs review rather than a rubber-stamp 'allow'. #8912's `allow-scan` action now
  // exists in this tree (package.name above uses it), but it is deliberately NOT used here:
  // allow-scan ships the real value minus whatever its shape scan and known-entity dictionary
  // catch, and neither catches an email address -- prescanAndMint would mint only the FQDN half
  // ("lists.ubuntu.com"), leaving the local part ("ubuntu-devel-discuss@") to reach the provider
  // verbatim, and scrubKnownEntities only knows values already minted this conversation. So
  // 'anonymize' remains the only action that does not ship a real maintainer address.
 ...
 * dynamically-built `RegExp` — a raw attacker/data-controlled value cannot be interpolated into a
 * regex source without first escaping it. TWO callers need it, for the same reason from opposite
 * ends: `Pseudonymizer.applyToText` below (#8916), which unlike a plain `split`/`join` must
 * express a word-boundary condition; and `scrubKnownEntities` (#8912), which needs real regex
 * features (case insensitivity plus the same boundary lookarounds). Kept as ONE definition
 * deliberately: #8916 and #8912 each introduced a copy of this helper, and having both in the
 * module is a duplicate-implementation error. */
 ...
 * - `scalar`: terms/significant_terms -- the bucket key is a single string, the value of `field`.
 *   NOT cardinality (or any other metric agg): see `resolveAggFieldSpec` for why mapping one
 *   leaks.
...
function resolveAggFieldSpec(
  ...
  // Bucket-producing types only — see `extractAggFields`'s doc comment for why metric types
  // (cardinality included) are deliberately NOT mapped. #8920: a leading
  // `cardinality(wazuh.agent.id -> allow)` declared before a
  // `terms(wazuh.agent.name -> anonymize)` would otherwise claim `samples[].key` and send real
  // hostnames to the provider verbatim under `wazuh.agent.id`'s 'allow' policy.
 ...
 *
 * Resolves `terms`/`significant_terms` (a single scalar field), `multi_terms` (an array of
 * fields, positionally aligned with the bucket key array), and `composite` (a
 ...
 *
 * - `scalar` (terms/significant_terms): the existing single-field decision, unchanged
...
export function applyFieldPolicy(
  ...
  // The spec a bucket row's `key` holds values against — see the `samples` note above. The first
  ...
  // extractable spec (e.g. only a date_histogram — whose bucket keys are NUMBERS and never reach
  // the string-scrub branches anyway), in which case `key` resolves by its own name like any
  // other sample field.
  const firstAggSpec = aggFields
    ? Object.values(aggFields).find(spec => spec !== undefined)
    : undefined;
