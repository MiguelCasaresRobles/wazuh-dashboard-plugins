...
export interface Digest {
   ...
   *
   * `key` is `unknown`, not `string`: a plain terms/significant_terms bucket's key is
   ...
   *
   * Omitted for a real aggregation whose buckets are complete (its COUNTS are always
   * population-true: OpenSearch computes `aggregations` over every matched doc regardless of
   * `size`), and for a synthetic one over an untruncated result (`returned === total`, where
   * grouping every returned row already IS grouping the full population).
   * `buildSamplesNote`'s "trust the breakdown" variant keys off
 ...
 * (array) and `composite` (object) bucket keys — a scalar `terms`/`significant_terms` key is still
 * always a plain string, unchanged). Caps every STRING leaf it finds and leaves any
