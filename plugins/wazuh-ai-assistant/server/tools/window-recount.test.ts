...
test('widenToDefaultWindow: a narrow ISO window widens to the plugin default window, size:0, no aggs', () => {
  ...
  // Intent only: applySafetyValves independently forces track_total_hits to `true` before
  // execution (no numeric clamp -- that cap was removed), so the EXECUTED recount is an exact
  // count with relation:'eq'. The executed behaviour (including the "at least N" fallback hint
  // wording for a lower-bound response) is pinned in executor.test.ts.
