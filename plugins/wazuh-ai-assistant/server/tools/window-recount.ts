 ...
 *
 * `track_total_hits: true` is set here as intent, and `applySafetyValves` independently forces the
 * same value before execution -- it no longer clamps to a numeric cap (that cap was removed
 * because it made `counts.total` under-report once a real match count crossed 10000), so the
 * EXECUTED recount is an exact count reporting `hits.total.relation: 'eq'`. The hint wording in
 * executor.ts still reads the relation and falls back to "at least N" for any lower-bound
 * response -- never an exact figure it does not have.
 *
