...
/** Test-only shorthand for the common `terms`/`significant_terms` case of `AggFieldSpec` — every
 * pre-existing test here predates `multi_terms`/`composite` support and only ever needs this one
 * shape. NOT `cardinality`: a metric agg resolves to `undefined` on purpose (#8920), so a test
 * about one must assert that, never `scalarSpec(...)`.
 *
 * Always build an `aggFields` map through this helper rather than as a plain `{agg: 'field'}`
 * string map: `AggFieldSpec` is a discriminated union, so a bare string resolves to no spec at
 * all at runtime and the test silently asserts nothing. */
...
test('applyFieldPolicy: get_agent_inventory packages breakdown anonymizes package.vendor buckets, not package.architecture', () => {
  ...
  // Built through `scalarSpec`, matching the `{kind: 'scalar', field: dimension}` specs
  // executor.ts's identity-map fallback actually produces since `AggFieldSpec` was widened for
  // multi_terms/composite (#8909) -- a plain `{dimension: dimension}` STRING map resolves to no
  // spec at all and would make this test assert nothing.
  const aggFields = {
    'package.architecture': scalarSpec('package.architecture'),
    'package.vendor': scalarSpec('package.vendor'),
  };
  const out = applyFieldPolicy(
    digest,
    FIELD_POLICY_DEFAULTS,
...
test('extractAggFields: maps each top-level BUCKET agg name to its terms/significant_terms field', () => {
  ...
  // doc comment for the leak this prevents. The entry is still PRESENT with an `undefined` spec,
  // which is what keeps the returned map aligned with the body's aggregation order.
