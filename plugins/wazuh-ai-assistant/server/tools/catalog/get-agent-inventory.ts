...
export const INVENTORY_KIND_CONFIG: Record<InventoryKind, InventoryKindConfig> =
  {
    ...
    ports: {
      ...
      // process.name, network.transport, destination.ip), followed by source.ip as the 6th
      // visible column; destination.port/process.pid are demoted -- NOT deleted, still queried
      // and still in the row expander -- to positions 7-8. destination.ip stays ahead of them: on
      // an established connection it carries the peer address, which is more often what a reader
      // wants than the local source.ip/the numeric process.pid.
      source: [
        'source.port',
        'interface.state',
        'process.name',
...
export const getAgentInventoryTool: ToolDefinition = {
  spec: {
    ...
    description:
      ...
      'To check whether one specific package/port/process is present (e.g. "is openssl ' +
      'installed on this host?", "what is listening on port 9200?"), pass "filter" ' +
      'instead of scanning the returned rows yourself -- results are truncated well ' +
      'before every row is returned (see "limit" below), so a specific entry can be ' +
      'absent from the sample even when it exists.',
