...
/** Picks which no-text fallback fits a turn that ended without any `delta` text and does NOT
 * qualify for (or fell through) the forced-synthesis retry below — shared by every `!sawAnyDelta`
 * exit point in `orchestrate` so this decision lives in exactly one place. The
 * `toolUsedThisTurn && sawNonEmptyTable` case (a real, non-empty table with no narration) no
 * longer resolves here: `emitNoTextFallback` intercepts that case first and only falls back to
 * NO_ANALYSIS_TEXT_MESSAGE if forced synthesis produced nothing AND there was no digest to derive
 * a truthful sentence from either (structurally unreachable in practice, since a non-empty table
 * always yields a `digest` event — kept as a defensive last resort, not a live path). */
...
export async function* orchestrate(
  ...
  async function* emitNoTextFallback(): AsyncGenerator<StreamEvent> {
    if (
      toolUsedThisTurn &&
      sawNonEmptyTable &&
